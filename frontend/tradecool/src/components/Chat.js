...
function Chat() {
  ...
  function refreshReceiverId() {
    receiverId = window.location.href.substring(
      29 + senderId.toString().length + productId.toString().length
    );
  }
        ...
        html = (
          <div>
            ...
            <select id={"input-select"}>
              {otherUsers &&
                otherUsers.map((user) => (
                  <option onClick={changeChat} value={user.id}>
                    {user.username}
                  </option>
                ))}
              <div key={otherUsers.id}></div>
            </select>
          </div>
        );
      }
